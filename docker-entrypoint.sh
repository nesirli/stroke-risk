...
# /app/data is a mounted volume (often empty on first run on Dokploy),
...
# If no champion model is registered yet (fresh volume or ephemeral deploy),
# train and promote one so /predict works out of the box.
if ! uv run python - <<'PY'
import mlflow
