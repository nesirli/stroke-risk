...
**Docker, not a cloud-specific runtime.** The whole app ships as a single Docker image, so it runs the same way locally, on CI, or on any container host. CI builds the image from this repo and pushes it to Docker Hub; Dokploy pulls that image and runs it.
...
The image is built by CI (`.github/workflows/ci.yml`) and pushed to Docker Hub, so the deploy host only pulls it.

1. Create an Application in Dokploy with **Provider: Docker**, image `nasirnesirli/stroke-risk:latest` (public).
2. Attach a volume at `/app/data` so the trained model and MLflow store survive redeploys. On first run the container seeds the dataset and tuned hyperparameters into the volume automatically, then trains and promotes a champion model. Without a volume the data is ephemeral and the container retrains on every deploy; the app still works.
3. Add the domain with container port `8000` and HTTPS. To serve at a subpath, set the domain **Path** to `/portfolio/stroke-risk` with **Strip Path** enabled, and set `ROOT_PATH=/portfolio/stroke-risk` so FastAPI and Gradio generate correctly prefixed URLs. For a dedicated subdomain, use the root path and leave `ROOT_PATH` empty.
4. Deploy. The app listens on `PORT` if set, otherwise `8000`, and the healthcheck hits `/health`.
